...
import { ChevronLeft, Loader2, User, Mail, Phone, MapPin, UserCheck } from 'lucide-react';
...
const CustomerFormPage = () => {
  ...
  }, [id, isEdit, fetchUsers, getCustomerById]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/api/customer/update/${id}`, formData);
      } else {
        await api.post('/api/customer/create', formData);
      }
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">พนักงานขายที่ดูแล</label>
              <div className="relative">
          ...
          <CardFooter className="flex justify-end gap-3 border-t border-gray-100 pt-6">
            <Button variant="outline" type="button" onClick={() => navigate('/admin/customers')} disabled={isLoading}>
              ยกเลิก
            </Button>
